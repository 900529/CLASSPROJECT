...
public class Calculator {
	...
	/*1.  main() 메소드를 정의하고 각각의 메소드를 호출해서 결과를 콘솔에 출력
	2. 콘솔에서 사용자에게 데이터를 받아 (입력) 메소드를 호출할 때, 받은 데이터를 메소드의 매개변수의 인자로 전달하는 코드를 main() 메소드 추가
	*/
	
	//① 정수 두 개를 매개변수의 인자로 전달받아 / 더하기연산 후 출력하는 / 메소드 정의 (반환없음 void) / addResultPrint 메소드 이름 설정 / 정수 long
	...
	//⑤ 실수 반지름 하나를 매개변수의 인자로 전달받아 /  원의 둘레를 구해 / 반환하는 메소드를 반환하는 메소드 정의  반지름을 float, 반환하는 double
	...
	//⑥ 실수 반지름 하나를 매개변수의 인자로 전달받아 / 원의 넓이를 구해 / 반환하는 메소드를 반환하는 메소드 정의  (원 둘레 : 2 x π x r , 월 넓이 : π x r x r)
	// 월 넓이 : π x r x r
