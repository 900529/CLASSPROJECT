-- 1. 덧셈연산자를 이용하여 모든 사원에 대해서 (행을 찾는 조건이 없다!) $300의 급여 인상을 계산한 후 사원의 이름, 급여, 인상된 급여를 출력하시오.
select ename, sal, sal+300 as ADD_SAL
from emp;
-- order by 3; 세번째 컬럼을 간단히 표현 가능 

-- 2. 사원의 이름, 급여, 연간 총 수입을 총 수입이 많은 것부터 작은 순으로 (내림차순) 출력하시오, 
...
from emp
where comm in not null and comm !=0
order by sal desc, comm desc;   --확인
​
​-- 12. 이름의 세번째 문자가 R인 
-- 사원의 이름을 표시하시오.
select ename
...
select ename, jab, sal
from emp
​where job in ('CLERK', 'SALESMAN') and sal not in ('1600', '950', '1300');  -- 오류??
-- ​where (job = 'CLERK' or job = 'SALESMAN') and sal not in (1600,950,1300);  -- 선생님 
